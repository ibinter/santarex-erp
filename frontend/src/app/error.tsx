...
const DICT = {
  fr: {
    ...
    description: "Une erreur inattendue s'est produite. Notre équipe technique en a été informée.",
    code: 'Code',
    retry: 'Réessayer',
  ...
  en: {
    ...
    description: 'An unexpected error occurred. Our technical team has been notified.',
    code: 'Code',
    retry: 'Try again',
    dashboard: 'Dashboard',
  },
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #0D1B2A 0%, #1A2F45 100%)', padding: 20 }}>
      <div style={{ textAlign: 'center', maxWidth: 520 }}>
        ...
        <h1 style={{ fontSize: 28, fontWeight: 800, color: '#fff', margin: '0 0 8px' }}>{t.title}</h1>
        <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.6)', margin: '0 0 32px', lineHeight: 1.6 }}>
          {t.description}
          {error?.digest && <span style={{ display: 'block', marginTop: 8, fontFamily: 'monospace', fontSize: 11, color: 'rgba(255,255,255,0.35)' }}>{t.code}: {error.digest}</span>}
        </p>
