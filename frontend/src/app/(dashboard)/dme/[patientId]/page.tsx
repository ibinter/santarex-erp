...
const TABS = [
  { id:'resume',        label:'Résumé',       icon:<Activity size={14}/> },
...
export default function DMEPage() {
  ...
  if (loading) return (
    <div style={{ padding:60, textAlign:'center', background:'#F4F6FA', minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16 }}>
      ...
      <p style={{ fontSize:14, color:'#546E7A', margin:0, fontWeight:600 }}>Chargement du dossier médical…</p>
    </div>
  );
    ...
    <div style={{ padding:'18px', background:'#F4F6FA', minHeight:'100vh' }}>
          ...
          <div style={{ flex:1, minWidth:200 }}>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:12, marginTop:8 }}>
              ...
              <span style={{ fontSize:12, color:'rgba(255,255,255,0.85)', fontWeight:600 }}>
                {patient.sexe==='M'?'♂ Homme':patient.sexe==='F'?'♀ Femme':'—'} • {age} ans
              </span>
              {patient.dateNaissance&&(
                <span style={{ fontSize:12, color:'rgba(255,255,255,0.7)', display:'flex', alignItems:'center', gap:4 }}>
                  <Calendar size={11}/> Né(e) le {fmtDate(patient.dateNaissance)}
                </span>
              )}
              {patient.telephone&&(
                <span style={{ fontSize:12, color:'rgba(255,255,255,0.7)', display:'flex', alignItems:'center', gap:4 }}>
            ...
            {patient.groupeSanguin&&(
              <div style={{ padding:'10px 16px', borderRadius:12, background:'rgba(255,255,255,0.15)', backdropFilter:'blur(8px)', border:'1px solid rgba(255,255,255,0.25)', textAlign:'center' }}>
                <div style={{ fontSize:9, color:'rgba(255,255,255,0.7)', textTransform:'uppercase', letterSpacing:'.8px', marginBottom:4, display:'flex', alignItems:'center', gap:4 }}>
                  <Droplets size={10}/> Groupe sanguin
                </div>
      ...
      <div style={{ display:'flex', gap:0, marginBottom:16, background:'#fff', borderRadius:12, padding:5, boxShadow:'0 1px 6px rgba(0,0,0,0.07)', overflowX:'auto' }}>
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)}
            style={{ display:'flex', alignItems:'center', gap:7, padding:'9px 18px', borderRadius:9, border:'none', background:tab===t.id?'linear-gradient(135deg,#0D47A1,#1565C0)':'transparent', color:tab===t.id?'#fff':'#546E7A', fontSize:13, fontWeight:tab===t.id?700:500, cursor:'pointer', whiteSpace:'nowrap', transition:'all .2s', position:'relative' }}>
            {t.icon} {t.label}
            {tabCounts[t.id]>0&&(
              <span style={{ background:tab===t.id?'rgba(255,255,255,0.25)':'#EFF6FF', color:tab===t.id?'#fff':'#1565C0', fontSize:10, fontWeight:800, padding:'1px 6px', borderRadius:10, marginLeft:2 }}>
                {tabCounts[t.id]}
              </span>
            )}
          </button>
      ...
      {tab==='resume'&&(
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:14, animation:'fadeUp .25s ease' }}>
          {[
            { label:'Consultations', count:consultations.length, icon:<Stethoscope size={20}/>, color:'#1565C0', bg:'linear-gradient(135deg,#EFF6FF,#DBEAFE)', border:'#BBDEFB', tab:'consultations' },
            { label:'Antécédents',   count:antecedents.length,   icon:<Heart size={20}/>,       color:'#C62828', bg:'linear-gradient(135deg,#FFEBEE,#FFCDD2)', border:'#FFCDD2', tab:'antecedents' },
            { label:'Analyses labo', count:analyses.length,      icon:<FlaskConical size={20}/>, color:'#6A1B9A', bg:'linear-gradient(135deg,#F3E5F5,#E1BEE7)', border:'#E1BEE7', tab:'analyses' },
            { label:'Ordonnances',   count:ordonnances.length,   icon:<Pill size={20}/>,         color:'#2E7D32', bg:'linear-gradient(135deg,#E8F5E9,#C8E6C9)', border:'#C8E6C9', tab:'ordonnances' },
          ].map(s=>(
          ...
          <div style={{ padding:'20px 22px', borderRadius:14, background:'#fff', border:'1.5px solid #E8EEFA', boxShadow:'0 2px 8px rgba(0,0,0,0.07)', gridColumn:'span 2' }}>
            <h3 style={{ margin:'0 0 14px', fontSize:13, fontWeight:800, color:'#1A2332', textTransform:'uppercase', letterSpacing:'.6px', display:'flex', alignItems:'center', gap:8 }}>
              <ClipboardList size={15} color="#1565C0"/> Informations générales
            </h3>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))', gap:10 }}>
              {[
                { label:'Statut', value: patient.statut==='actif'?'✓ Actif':'Inactif', color: patient.statut==='actif'?'#2E7D32':'#546E7A' },
                { label:'Sexe', value: patient.sexe==='M'?'♂ Homme':patient.sexe==='F'?'♀ Femme':'—' },
                { label:'Âge', value:`${age} ans` },
                { label:'Groupe sanguin', value:patient.groupeSanguin||'—', color:sangColor },
                { label:'Téléphone', value:patient.telephone||'—' },
                { label:'Date de naissance', value:fmtDate(patient.dateNaissance) },
              ].map(info=>(
        ...
        <div style={{ animation:'fadeUp .25s ease' }}>
          {antecedents.length===0?(
            <div style={{ background:'#fff', borderRadius:14, padding:'50px 20px', textAlign:'center', boxShadow:'0 1px 6px rgba(0,0,0,0.07)' }}>
              ...
              <p style={{ margin:0, color:'#90A4AE', fontSize:13, fontWeight:600 }}>Aucun antécédent enregistré</p>
            </div>
          ):(
                  ...
                  <div key={a.id} className="ant-card"
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ display:'flex', gap:8, marginBottom:6, flexWrap:'wrap' }}>
                        <span style={{ fontSize:11, fontWeight:800, padding:'3px 10px', borderRadius:20, background:cfg.bg, color:cfg.color, border:`1px solid ${cfg.border}` }}>{cfg.label}</span>
                        <span style={{ fontSize:11, fontWeight:800, padding:'3px 10px', borderRadius:20, background:gcfg.bg, color:gcfg.color, border:`1px solid ${gcfg.border}` }}>{gcfg.label}</span>
                      </div>
                      <div style={{ fontSize:14, fontWeight:700, color:'#1A2332', marginBottom:a.traitement?4:0 }}>{a.description}</div>
                      {a.traitement&&<div style={{ fontSize:12, color:'#546E7A', display:'flex', alignItems:'center', gap:5 }}><Pill size={11} color="#546E7A"/> Traitement : {a.traitement}</div>}
                    </div>
        ...
        <div style={{ animation:'fadeUp .25s ease' }}>
          {consultations.length===0?(
            <div style={{ background:'#fff', borderRadius:14, padding:'50px 20px', textAlign:'center', boxShadow:'0 1px 6px rgba(0,0,0,0.07)' }}>
              ...
              <p style={{ margin:0, color:'#90A4AE', fontSize:13, fontWeight:600 }}>Aucune consultation enregistrée</p>
            </div>
          ):(
              ...
              {consultations.map(c=>{
                ...
                return (
                  <div key={c.id} className="con-card" onClick={()=>router.push(`/consultations/${c.id}`)}
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:13, fontWeight:700, color:'#1A2332' }}>{c.motif??'Consultation'}</div>
                      ...
                      {c.diagnostic&&<div style={{ fontSize:11, color:'#546E7A', marginTop:4, fontStyle:'italic' }}>Diagnostic : {c.diagnostic}</div>}
                    </div>
        ...
        <div style={{ animation:'fadeUp .25s ease' }}>
          {ordonnances.length===0?(
            <div style={{ background:'#fff', borderRadius:14, padding:'50px 20px', textAlign:'center', boxShadow:'0 1px 6px rgba(0,0,0,0.07)' }}>
              ...
              <p style={{ margin:0, color:'#90A4AE', fontSize:13, fontWeight:600 }}>Aucune ordonnance enregistrée</p>
            </div>
          ):(
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {ordonnances.map(o=>(
        ...
        <div style={{ animation:'fadeUp .25s ease' }}>
          {analyses.length===0?(
            <div style={{ background:'#fff', borderRadius:14, padding:'50px 20px', textAlign:'center', boxShadow:'0 1px 6px rgba(0,0,0,0.07)' }}>
              ...
              <p style={{ margin:0, color:'#90A4AE', fontSize:13, fontWeight:600 }}>Aucune analyse enregistrée</p>
            </div>
          ):(
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {analyses.map(a=>{
                ...
                return (
                  <div key={a.id} className="ana-card"
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:13, fontWeight:700, color:'#1A2332' }}>{a.typeAnalyse??'Analyse'}</div>
