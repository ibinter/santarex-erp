...
const STEPS = ['Modèle', 'Fichier', 'Aperçu', 'Import'];
...
export default function ImportPatientsPage() {
  ...
  const pickFile = (f: File | null | undefined) => {
    ...
    if (ext !== '.xlsx' && ext !== '.csv') { setError('Format non supporté. Utilisez un fichier .xlsx ou .csv.'); return; }
    if (f.size > MAX_BYTES) { setError('Fichier trop volumineux (maximum 5 Mo).'); return; }
    setFile(f);
  };
  ...
  const telechargerModele = useCallback(async () => {
    ...
    } catch (e: any) { setError(e?.message || 'Erreur lors du téléchargement du modèle.'); }
  ...
  const lancerApercu = useCallback(async () => {
    ...
    try {
      ...
      setPreview(data);
    } catch (e: any) { setError(e?.message || 'Erreur lors de l\'analyse du fichier.'); }
    ...
    <div style={{ padding: 18, background: '#F4F6FA', minHeight: '100vh' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#0A2E6E 0%,#1565C0 60%,#0288D1 100%)', borderRadius: 18, padding: '20px 26px', marginBottom: 18, boxShadow: '0 8px 28px rgba(13,71,161,0.3)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: 21, fontWeight: 900, color: '#fff', letterSpacing: '-0.3px' }}>Import de patients</h1>
            <p style={{ margin: '3px 0 0', fontSize: 12, color: 'rgba(255,255,255,0.75)', fontWeight: 500 }}>
              Importez vos patients en masse depuis un fichier Excel ou CSV
            </p>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 18, flexWrap: 'wrap' }}>
        {STEPS.map((s, i) => (
          <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {i < STEPS.length - 1 && <ChevronRight size={14} color="#B0BEC5" />}
          </div>
        ))}
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 14, marginBottom: 16 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 14, padding: 20, boxShadow: '0 1px 6px rgba(0,0,0,0.07)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 800, color: '#1A2332' }}>1. Téléchargez le modèle</div>
            </div>
            <p style={{ margin: '0 0 14px', fontSize: 12, color: '#546E7A', lineHeight: 1.5 }}>
              Le modèle XLSX contient les colonnes attendues (les champs suivis d'une <strong>*</strong> sont obligatoires : nom, prénom, date de naissance, sexe) et une feuille d'instructions.
            </p>
            <button onClick={telechargerModele} disabled={loadingModele}
              ...
              {loadingModele ? <Loader2 size={15} style={{ animation: 'spin 1s linear infinite' }} /> : <Download size={15} />} Télécharger le modèle
            </button>
          </div>
          ...
          <div style={{ background: '#fff', borderRadius: 14, padding: 20, boxShadow: '0 1px 6px rgba(0,0,0,0.07)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 800, color: '#1A2332' }}>2. Sélectionnez votre fichier</div>
            </div>
            <div
              ...
              {file ? (
                ...
                <>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#37474F' }}>Glissez un fichier ici ou cliquez</div>
                  <div style={{ fontSize: 11, color: '#90A4AE', marginTop: 3 }}>.xlsx ou .csv — max 5 Mo</div>
                </>
              )}
            </div>
            {file && !preview && (
              <button onClick={lancerApercu} disabled={loadingPreview}
                ...
                {loadingPreview ? <Loader2 size={15} style={{ animation: 'spin 1s linear infinite' }} /> : <FileSpreadsheet size={15} />} Analyser le fichier
              </button>
            )}
          </div>
        </div>
      ...
      {preview && !result && (
        <div style={{ animation: 'fadeUp .25s ease' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(160px,1fr))', gap: 12, marginBottom: 16 }}>
            {[
              { label: 'Lignes lues', value: preview.totalLignes, color: '#1565C0', bg: '#EFF6FF', border: '#BBDEFB' },
              { label: 'Valides', value: preview.lignesValides.length, color: '#2E7D32', bg: '#E8F5E9', border: '#C8E6C9' },
              { label: 'Doublons', value: preview.doublons.length, color: '#E65100', bg: '#FFF3E0', border: '#FFE0B2' },
              { label: 'En erreur', value: preview.lignesEnErreur.length, color: '#C62828', bg: '#FFEBEE', border: '#FFCDD2' },
            ].map((k, i) => (
          ...
          <div style={{ background: '#fff', borderRadius: 12, padding: '12px 16px', marginBottom: 14, boxShadow: '0 1px 6px rgba(0,0,0,0.06)', fontSize: 12, color: '#546E7A' }}>
            <strong style={{ color: '#2E7D32' }}>Colonnes reconnues :</strong> {preview.colonnesReconnues.join(', ') || '—'}
            {preview.colonnesIgnorees.length > 0 && (
              <div style={{ marginTop: 6 }}><strong style={{ color: '#E65100' }}>Ignorées :</strong> {preview.colonnesIgnorees.join(', ')}</div>
            )}
          </div>
          ...
          {preview.lignesEnErreur.length > 0 && (
            <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 1px 6px rgba(0,0,0,0.07)', marginBottom: 14, overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', background: '#FFEBEE', color: '#C62828', fontSize: 13, fontWeight: 800, display: 'flex', alignItems: 'center', gap: 8 }}>
                <AlertTriangle size={15} /> {preview.lignesEnErreur.length} ligne(s) en erreur (non importées)
              </div>
              <div style={{ maxHeight: 240, overflowY: 'auto' }}>
                {preview.lignesEnErreur.map(e => (
                  <div key={e.ligne} style={{ padding: '9px 16px', borderTop: '1px solid #F5F7FA', fontSize: 12, display: 'flex', gap: 10 }}>
                    <span style={{ fontWeight: 800, color: '#C62828', minWidth: 60 }}>Ligne {e.ligne}</span>
          ...
          {preview.doublons.length > 0 && (
            <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 1px 6px rgba(0,0,0,0.07)', marginBottom: 14, overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', background: '#FFF3E0', color: '#E65100', fontSize: 13, fontWeight: 800, display: 'flex', alignItems: 'center', gap: 8 }}>
                <Copy size={15} /> {preview.doublons.length} doublon(s) détecté(s)
              </div>
              <div style={{ maxHeight: 200, overflowY: 'auto' }}>
                {preview.doublons.map(d => (
                  <div key={d.ligne} style={{ padding: '9px 16px', borderTop: '1px solid #F5F7FA', fontSize: 12, display: 'flex', gap: 10, alignItems: 'center' }}>
                    <span style={{ fontWeight: 800, color: '#E65100', minWidth: 60 }}>Ligne {d.ligne}</span>
                    ...
                    <span style={{ marginLeft: 'auto', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 12, background: d.source === 'base' ? '#FFE0B2' : '#FFECB3', color: '#E65100' }}>
                      {d.source === 'base' ? 'déjà en base' : 'doublon fichier'}
                    </span>
                  </div>
                ))}
              </div>
              <div style={{ padding: '12px 16px', borderTop: '1px solid #F0F4FA', display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: '#546E7A' }}>Que faire des doublons ?</span>
                {([['ignorer', 'Ignorer'], ['creer', 'Importer quand même']] as const).map(([k, lbl]) => (
          ...
          {preview.lignesValides.length > 0 && (
            <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 1px 6px rgba(0,0,0,0.07)', marginBottom: 16, overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', background: 'linear-gradient(90deg,#F8FAFC,#F0F6FF)', fontSize: 13, fontWeight: 800, color: '#37474F', display: 'flex', alignItems: 'center', gap: 8 }}>
                <Users size={15} color="#1565C0" /> Aperçu des lignes valides ({preview.lignesValides.length})
              </div>
              <div style={{ overflowX: 'auto', maxHeight: 320 }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 620 }}>
                  <thead>
                    <tr style={{ background: '#FAFBFC' }}>
                      {['#', 'Nom', 'Prénom', 'Naissance', 'Sexe', 'Téléphone', 'Ville', 'Groupe'].map(h => (
              ...
              {preview.lignesValides.length > 100 && (
                <div style={{ padding: '8px 16px', fontSize: 11, color: '#90A4AE', borderTop: '1px solid #F0F4FA' }}>… et {preview.lignesValides.length - 100} autre(s) ligne(s).</div>
              )}
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            <button onClick={confirmer} disabled={loadingConfirm || nbAImporter <= 0}
              ...
              Confirmer l'import ({nbAImporter} patient{nbAImporter > 1 ? 's' : ''})
            </button>
      ...
      {result && (
        <div style={{ background: '#fff', borderRadius: 16, padding: 28, boxShadow: '0 2px 12px rgba(0,0,0,0.08)', textAlign: 'center', animation: 'fadeUp .3s ease' }}>
          ...
          <h2 style={{ margin: '0 0 6px', fontSize: 19, fontWeight: 900, color: '#1A2332' }}>Import terminé</h2>
          <p style={{ margin: '0 0 20px', fontSize: 13, color: '#546E7A' }}>
            {result.crees} patient(s) créé(s), {result.ignores} ignoré(s), {result.erreurs} en erreur.
          </p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 22 }}>
            {[
              { label: 'Créés', value: result.crees, color: '#2E7D32', bg: '#E8F5E9' },
              { label: 'Ignorés', value: result.ignores, color: '#E65100', bg: '#FFF3E0' },
              { label: 'Erreurs', value: result.erreurs, color: '#C62828', bg: '#FFEBEE' },
            ].map((k, i) => (
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={() => router.push('/patients')}
              ...
              <Users size={16} /> Voir les patients
            </button>
            <button onClick={reset}
