...
export default function ModifierPatientPage() {
  ...
  useEffect(() => {
    if (!id) return;
    setLoadingData(true);
    apiClient<any>(`/patients/${id}`).then(p => {
      setForm({
        nom: p.nom ?? '', prenom: p.prenom ?? '',
        dateNaissance: p.dateNaissance ? p.dateNaissance.slice(0, 10) : '',
        sexe: p.sexe ?? 'M',
        telephone: p.telephone ?? '', telephoneUrgence: p.telephoneUrgence ?? '',
        adresse: p.adresse ?? '', ville: p.ville ?? '', pays: p.pays ?? 'CI',
        groupeSanguin: p.groupeSanguin ?? '', allergies: p.allergies ?? '', antecedents: p.antecedents ?? '',
        assuranceNom: p.assuranceNom ?? '', assuranceNumero: p.assuranceNumero ?? '',
  ...
  if (error && loadingData === false && !form.nom) return (
    <div style={{ padding: 32, textAlign: 'center', color: '#C62828' }}>{error}</div>
  );
