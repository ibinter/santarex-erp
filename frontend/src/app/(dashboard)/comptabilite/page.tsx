...
export default function ComptabilitePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (firstErr) setError(firstErr.reason?.message ?? 'Erreur de chargement');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const submit = async (validerApres: boolean) => {
    setFormErr(null);
    if (!form.libelle.trim()) { setFormErr('Le libellé de l’écriture est requis.'); return; }
    ...
    if (lignes.length < 2) { setFormErr('Au moins deux lignes imputées sont nécessaires.'); return; }
    if (!equilibre) { setFormErr('L’écriture doit être équilibrée (Σ débit = Σ crédit).'); return; }
    ...
    try {
      ...
      setFormErr(e?.message ?? 'Échec de l’enregistrement de l’écriture.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const validerEcriture = async (id: string) => {
    try {
      ...
      setError(e?.message ?? 'Échec de la validation.');
    }
  };
  ...
  return (
    <div style={{ padding: '18px', background: '#F4F6FA', minHeight: '100vh' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#1A1A2E 0%,#16213E 50%,#0F3460 100%)', borderRadius: 18, padding: '24px 28px 20px', marginBottom: 20, position: 'relative', overflow: 'hidden', boxShadow: '0 8px 28px rgba(15,52,96,0.4)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div>
                ...
                <p style={{ margin: '3px 0 0', fontSize: 12, color: 'rgba(255,255,255,0.6)', fontWeight: 600 }}>
                  Journal des écritures · Grand livre · Bilan financier · SYSCOHADA
                </p>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={load} disabled={loading}
                ...
                <RefreshCw size={14} style={{ animation: loading ? 'spin 1s linear infinite' : undefined }} /> Actualiser
              </button>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 16 }}>
            {[
              { label: 'Produits', val: loading ? '…' : fmt(produits), icon: <TrendingUp size={11} /> },
              { label: 'Charges', val: loading ? '…' : fmt(charges), icon: <TrendingDown size={11} /> },
              { label: 'Résultat net', val: loading ? '…' : fmt(resultat), icon: <DollarSign size={11} /> },
              { label: 'Brouillons', val: loading ? '…' : nbAttente, icon: <FileText size={11} /> },
            ].map((s, i) => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'Produits', value: loading ? '…' : fmt(produits), color: '#065F46', bg: '#D1FAE5', border: '#A7F3D0', icon: <TrendingUp size={20} color="#065F46" />, tab: 'bilan' as const },
          { label: 'Charges', value: loading ? '…' : fmt(charges), color: '#991B1B', bg: '#FEE2E2', border: '#FECACA', icon: <TrendingDown size={20} color="#991B1B" />, tab: 'bilan' as const },
          { label: 'Résultat net', value: loading ? '…' : fmt(resultat), color: resultat >= 0 ? '#065F46' : '#991B1B', bg: resultat >= 0 ? '#D1FAE5' : '#FEE2E2', border: resultat >= 0 ? '#A7F3D0' : '#FECACA', icon: <DollarSign size={20} color={resultat >= 0 ? '#065F46' : '#991B1B'} />, tab: 'bilan' as const },
          { label: 'Écritures en brouillon', value: loading ? '…' : nbAttente, color: '#92400E', bg: '#FEF3C7', border: '#FDE68A', icon: <FileText size={20} color="#92400E" />, tab: 'journal' as const },
        ].map((k, i) => (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 760 }}>
              <thead>
                <tr style={{ background: '#F8FAFC' }}>
                  {['Réf.', 'Date', 'Journal', 'Libellé', 'Comptes', 'Débit (XOF)', 'Crédit (XOF)', 'Statut', ''].map(h => (
              ...
              <tbody>
                {loading && (
                  <tr><td colSpan={9} style={{ padding: '32px', textAlign: 'center', color: '#90A4AE', fontSize: 13 }}>Chargement…</td></tr>
                )}
      ...
      {tab === 'grand-livre' && (
        <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 1px 8px rgba(0,0,0,0.08)', overflow: 'hidden', animation: 'fadeUp .25s ease' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1.5px solid #EEF2F8', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: '#90A4AE', textTransform: 'uppercase', letterSpacing: '0.8px' }}>Balance générale — Plan SYSCOHADA</span>
            {balance && (
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 560 }}>
              <thead>
                <tr style={{ background: '#F8FAFC' }}>
                  {['Code', 'Intitulé du compte', 'Débit (XOF)', 'Crédit (XOF)', 'Solde (XOF)'].map(h => (
                    <th key={h} style={{ padding: '11px 16px', textAlign: h === 'Code' || h === 'Intitulé du compte' ? 'left' : 'right', fontSize: 10, fontWeight: 700, color: '#78909C', textTransform: 'uppercase', letterSpacing: '0.6px', borderBottom: '1.5px solid #EEF2F8' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {loading && (
                  <tr><td colSpan={5} style={{ padding: '32px', textAlign: 'center', color: '#90A4AE', fontSize: 13 }}>Chargement…</td></tr>
                )}
                {!loading && (!balance || balance.lignes.length === 0) && (
                  <tr><td colSpan={5} style={{ padding: '40px', textAlign: 'center', color: '#90A4AE', fontSize: 13 }}>
                    Aucun mouvement comptable. Les comptes apparaîtront après validation d’écritures.
                  </td></tr>
                )}
                ...
                {!loading && balance && balance.lignes.length > 0 && (
                  <tr style={{ borderTop: '2px solid #E2E8F0', background: '#F8FAFC' }}>
                    <td colSpan={2} style={{ padding: '14px 16px', fontSize: 13, fontWeight: 900, color: '#1A2332' }}>TOTAUX</td>
      ...
      {tab === 'bilan' && (
        <div style={{ animation: 'fadeUp .25s ease' }}>
          <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 1px 8px rgba(0,0,0,0.08)', padding: '16px 20px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: '#1A2332' }}>Bilan au {fmtDate(new Date().toISOString())}</span>
            {bilan && (
              <span style={{ marginLeft: 'auto', fontSize: 11, fontWeight: 700, background: bilan.equilibre ? '#D1FAE5' : '#FEE2E2', color: bilan.equilibre ? '#065F46' : '#991B1B', padding: '3px 12px', borderRadius: 20 }}>
                {bilan.equilibre ? '✓ Équilibré' : '✗ Non équilibré'}
              </span>
            )}
          </div>

          {loading && (
            <div style={{ background: '#fff', borderRadius: 14, padding: '40px', textAlign: 'center', color: '#90A4AE', fontSize: 13 }}>Chargement…</div>
          )}

          {!loading && bilan && (bilan.actif.length > 0 || bilan.passif.length > 0) && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              {[
                { titre: 'ACTIF', couleur: '#991B1B', bg: '#FEE2E2', accent: '#FECACA', postes: bilan.actif, total: bilan.totalActif },
                { titre: 'PASSIF', couleur: '#065F46', bg: '#D1FAE5', accent: '#A7F3D0', postes: bilan.passif, total: bilan.totalPassif },
              ].map(side => {
                  ...
                  <div key={side.titre} style={{ background: '#fff', borderRadius: 14, boxShadow: '0 1px 8px rgba(0,0,0,0.08)', overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '12px 16px' }}>
                      {side.postes.length === 0 && (
                        <div style={{ padding: '16px 0', fontSize: 12, color: '#90A4AE', textAlign: 'center' }}>Aucun poste.</div>
                      )}
                      {side.postes.map(p => {
          ...
          {!loading && bilan && bilan.actif.length === 0 && bilan.passif.length === 0 && (
            <div style={{ background: '#fff', borderRadius: 14, padding: '40px', textAlign: 'center', color: '#90A4AE', fontSize: 13 }}>
              Aucune donnée de bilan. Validez des écritures pour alimenter le bilan.
            </div>
          )}
        </div>
      )}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div style={{ background: 'linear-gradient(135deg,#16213E,#0F3460)', padding: '18px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <h2 style={{ margin: 0, fontSize: 17, fontWeight: 900, color: '#fff' }}>Nouvelle écriture comptable</h2>
              </div>
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 12, marginBottom: 16 }}>
                ...
                <div>
                  <label style={{ fontSize: 11, fontWeight: 700, color: '#64748B', display: 'block', marginBottom: 5 }}>Journal</label>
                ...
                <div>
                  <label style={{ fontSize: 11, fontWeight: 700, color: '#64748B', display: 'block', marginBottom: 5 }}>Référence</label>
                  <input className="compta-input" placeholder="Pièce / réf." value={form.reference} onChange={e => setForm(f => ({ ...f, reference: e.target.value }))} />
                </div>
              </div>
              <div style={{ marginBottom: 18 }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#64748B', display: 'block', marginBottom: 5 }}>Libellé de l’écriture *</label>
                <input className="compta-input" placeholder="Ex. Recettes consultations — Juillet" value={form.libelle} onChange={e => setForm(f => ({ ...f, libelle: e.target.value }))} />
              </div>

              {/* Lignes */}
              <div style={{ marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 12, fontWeight: 800, color: '#334155', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Lignes d’imputation</span>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 640 }}>
                    ...
                    {form.lignes.map((l, idx) => (
                      <tr key={idx}>
                        ...
                        <td style={{ padding: '4px 8px', minWidth: 160 }}>
                          <input className="compta-input" placeholder="(libellé écriture)" value={l.libelle} onChange={e => setLigne(idx, { libelle: e.target.value })} />
                        </td>
                  ...
                  <tfoot>
                    <tr style={{ borderTop: '2px solid #E2E8F0' }}>
                      <td colSpan={2} style={{ padding: '10px 8px', fontSize: 12, fontWeight: 800, color: '#334155', textAlign: 'right' }}>Totaux</td>
              ...
              <div style={{ marginTop: 12, padding: '10px 14px', borderRadius: 10, background: equilibre ? '#ECFDF5' : '#FEF2F2', border: `1.5px solid ${equilibre ? '#A7F3D0' : '#FECACA'}`, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 12, fontWeight: 800, color: equilibre ? '#065F46' : '#991B1B' }}>
                  {equilibre ? '✓ Écriture équilibrée' : `✗ Déséquilibre : ${num(Math.abs(totalDebit - totalCredit))} XOF`}
                </span>
              </div>
